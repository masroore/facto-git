...
class ManagerEdit extends Component
{
    ...
    public $post_cat_id;
    public $photos = [] , $mainphoto , $torrentimages, $torrentfile , $userfiles,  $magnet_link, $upload_files, $thumb, $upsothumb ;
    public $post_cat, $tags;
    
    public $progress , $progress_thumb , $progress_upso_thumb , $progressMainphoto,  $progressTorrentfile,$progressUserfiles, $is_saving ; 
    ...
    public $min_reward_points, $reward_points, $testme ; 

    public $main_region_id, $upso_type_id, $search;
    public $show_order, $content, $site_name, $site_url, $phone, $region_id,  $title ;

    public $upso, $name, $price, $age, $ht, $wt, $bsize, $cc;
    ...
    function get_min_reward_points( $post_cat ){
        $value_data = unserialize( $post_cat->value);
        $this->reward_points = $this->min_reward_points =  (int) $value_data['min_reward_points'];
    }

    function initData(){
        $this->manager = Manager::find( $this->manager_id);

        $this->name=$this->manager->name ;
        $this->age= $this->manager->age;
        $this->wt= $this->manager->wt;
        $this->ht= $this->manager->ht;
        $this->cc= $this->manager->cc;
        $this->price = $this->manager->price;
        $this->bsize= $this->manager->bsize;
        $this->content = $this->manager->content; 
        $this->content = strip_tags( $this->content, '<img><p><br><div>')  ;

        $allowances = $this->manager->allowances->pluck('id');
        $this->allowances = [];
        foreach ( $allowances as $allowance) {
            $this->allowances[] = (string) $allowance;
        }
    ...
    function updatedAllowances( $value){
        // dd($value);
    }
    ...
    function save( ){
        ...
        if( $this->removeUserImages) {
            $this->doRemovePhotos($manager ) ;
        } else if( $this->photos && count( $this->photos) > 0 ){
    ...
    protected function saveCache($manager ){
    ...
    protected function doRemovePhotos( $manager ){
        $manager->all_images()->delete();
    }
    ...
    protected function insertManagerTumb( $manager, $thumb ){
    ...
    {
        $this->validate([
            'upsothumb' =>  'file|mimes:jpeg,bmp,png|max:2048',
        ]);
    }

    function updatingThumb(){
    ...
    {
        $this->validate([
            'thumb' =>  'file|mimes:jpeg,bmp,png|max:2048',
        ]);
    }
    ...
    function permissionCheck( $mode,   $post_cat ){
    ...
    function htmlCheck( $mode='html',   $post_cat_id ){
    ...
    function writingCountLimitCheck( $user, $post_cat_id ){
    ...
    protected function get_sub_region_ids($main_region_id, $sub_region_id ){
        ...
        if( $sub_region_id ) {
            ...
            if( ! $main_region_id ){
                return Region::whereNotNull('parent_id' )->get()->pluck('id')->toArray();
            }  else {
    ...
    protected function get_upsos($upso_type_id, $sub_region_ids, $show_order=3, $search ){
        if( $show_order == 1 ){
