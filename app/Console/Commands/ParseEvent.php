...
class ParseEvent extends Command
{
    ...
    protected $page, $activeSports, $allSports, $betting_link  ;
    ...
    function getIframeHTML( $event){
        ...
        foreach( $divs as $div){
            if( $div->id == 'div_Btn_DS') {
                array_push($bet_type_btns, 'ds');
            } else if( $div->id == 'div_Btn_BS') {
                array_push($bet_type_btns, 'bs');
            } else if( $div->id == 'div_Btn_SS') {
                array_push($bet_type_btns, 'ss');
            } else if( $div->id == 'div_Btn_SBS') {
                array_push($bet_type_btns, 'sbs');

            } else if( $div->id == 'div_Btn_SSS') {
                array_push($bet_type_btns, 'sss');
            } else if( $div->id == 'div_Btn_BSJ') {
                array_push($bet_type_btns, 'bsj');
            } else if( $div->id == 'div_Btn_SSJ') {
                array_push($bet_type_btns, 'ssj');
            } else if( $div->id == 'div_Btn_SBJ') {
                array_push($bet_type_btns, 'sbj');
            }
        }
            ...
            foreach( $trs as $tr ) {
                ...
                foreach( $tds  as $td ){
                    switch ($i) {
                        ...
                        case 1:
                            $data['win1'] = trim($td->plaintext);break;
                        case 2:
                            $data['win2'] = trim($td->plaintext);break;
                        case 3:
                            $data['win3'] = trim($td->plaintext);break;
                        case 4:
                            $data['odds_ds'] = trim($td->plaintext);break;
                        case 5:
                            $data['odds_bs'] = trim($td->plaintext);break;
                        case 6:
                            $data['odds_ss'] = trim($td->plaintext);break;
                        case 7:
                            echo 'odds_bys' ;
                            echo "\n";
                            echo trim($td->plaintext);
                            echo "\n";
                            $data['odds_bys'] = trim($td->plaintext);break;
                            // 복연승 1/2/1.5
                        case 8:
                            $data['odds_sbs'] = trim($td->plaintext);break;     
                            // 삼복승 
                        case 9:
                            $data['odds_sss'] = trim($td->plaintext);break;     
                            // 삼쌍승
                    }
        ...
        if ( $this->page->querySelector( $selector ) )
        {
            ...
            foreach( $trs as $tr ) {
                ...
                foreach( $tds  as $td ){
                    switch ($i) {
                        case 0:
                            $data['sport'] = trim($td->plaintext);break;
                        case 1:
                            $data['area'] = trim($td->plaintext);break;
                        case 2:
                            $data['round'] = trim($td->plaintext);break;
                        case 3:
                            $data['numbers'] = trim($td->plaintext);break;
                        case 4:
                            $data['distance'] = trim($td->plaintext);break;
                        case 5:
                            $data['status'] = trim($td->plaintext);break;
                        case 6:
                            $data['closing'] = trim($td->plaintext);break;
                        case 7:
                            $data['event_time_org'] = $t_org = trim($td->plaintext);
                            $t_org= $t_org . ":00";
                            $event_time = $check_date . " " . $t_org;
                            $data['event_time'] = $event_time ; break;
                        case 8:
                            $data['remain_time'] = trim($td->plaintext);break;
                            break;
                        case 9:
    ...
    function init(){
        ...
        foreach( $dom->find('ul.a-btn-group > li')  as $li ){
            ...
            if( Str::contains( $li_text, 'Live') ){
                array_push( $sports_tmp, trim( str_replace( 'Live', '' ,$li_text ))  );
            }
        }
        $sports = Sport::all(); ;  
        foreach( $sports as $sport){
            if( in_array( $sport->name, $sports_tmp ) ) {
                array_push( $this->activeSports , $sport);
            }
        }
