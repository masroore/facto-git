...
class ManagerController extends Controller
{
    ...
    function update( Upso $upso, Request $request ){
        dd($upso);
    }
    ...
    function get_managers( $region_ids, $upso_type_id, $allow_ids  ){
        $managers = Manager::whereHas('upso', function($q) use( $region_ids, $upso_type_id) {
                                $q->whereIn('region_id', $region_ids)
                                    ->where('upso_type_id', $upso_type_id);
                    })->when( $allow_ids, function($q, $allow_ids){
                                return $q->whereHas('allowances', function( $qq) use( $allow_ids) {
                                    $qq->whereIn('id', $allow_ids);
                                });
                    })->get();
    }

    function show( Manager $manager, Request $request){
    ...
    function test( Manager $manager, Request $request){
    ...
    function getPostBest(  $type, $post_cat_ids, $count  ) {
    ...
    protected function get_upsos($upso_type_id, $region_ids, $show_order=3, $search ){
