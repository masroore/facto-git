...
class UpsoPage extends Component
{
    ...
    public function render()
    {
        ...
        if (! $this->main_region_id) {
            ...
            $sub_regions = Region::where('parent_id', $this->main_region_id)
                            ->with(['upsos'=> function ($q) use ($upso_type_id) {
        ...
        if ($this->search) {
            $upsos_best = $upsos_best->where(function ($q) {
                $q->where('site_name', 'like', '%'.$this->search.'%')
                    ->orWhere('title', 'like', '%'.$this->search.'%');
            });
            $upsos = $upsos->where(function ($q) {
                $q->where('site_name', 'like', '%'.$this->search.'%')
                    ->orWhere('title', 'like', '%'.$this->search.'%');
            });
        }
    ...
    public function mount($upso_type_id, $main_region_id, $region_id, Request $request)
    {
        // dd($upso_type_id);
    ...
    public function setMainRegion($id)
    {
        if ($id == -1) {
    ...
    public function setSubRegion($id)
    {
        $this->region_id = $id;
    }
